...
@Component({
  selector: 'app-outras-producoes-bibliograficas',
  standalone: true,
  imports: [
    NgFor,
    NgIf,
  ...
  templateUrl: './outras-producoes-bibliograficas.component.html',
  styleUrl: './outras-producoes-bibliograficas.component.css'
})
export class OutrasProducoesBibliograficasComponent {
  curriculumns: ICurriculum[] = []
  sections: Artigo[] = []
  tableElements: ITableElements[] = [
    {
      title: 'Outras produções bibliográficas',
      property: 'outrasProducoesBibliograficas'
    }
  ]
  ...
  countSections(curriculum: ICurriculum): number {
    return curriculum.curriculum.outrasProducoesBibliograficas.length
  }
  ...
  getWorks(): void {
    this.sections = this.curriculumns.reduce<Artigo[]>(
      (trabalhos, curriculum) =>
        trabalhos.concat(
          curriculum.curriculum.outrasProducoesBibliograficas.map(
            (section) => ({
              ...section,
              ano: section.ano,
