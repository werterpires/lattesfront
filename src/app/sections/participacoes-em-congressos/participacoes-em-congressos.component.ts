...
@Component({
  selector: 'app-participacoes-em-congressos',
  standalone: true,
  imports: [
    NgFor,
    NgIf,
  ...
  templateUrl: './participacoes-em-congressos.component.html',
  styleUrl: './participacoes-em-congressos.component.css'
})
export class ParticipacoesEmCongressosComponent {
  curriculumns: ICurriculum[] = []
  participations: Participacao[] = []
  tableElements: ITableElements[] = [
    {
      title: 'Participações em congressos',
      property: 'participaçõesEmCongressos'
    }
  ]
  ...
  countOutrasParticipacoesEmEventos(curriculum: ICurriculum): number {
    return curriculum.curriculum.participacoesEmCongressos.length
  }
    ...
    this.participations = this.curriculumns.reduce<Participacao[]>(
      (participations, curriculum) =>
        participations.concat(
          curriculum.curriculum.participacoesEmCongressos.map(
            (participation) => ({
