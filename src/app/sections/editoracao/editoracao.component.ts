...
import { Editoracao } from 'src/app/shared/services/objTypes'
...
@Component({
  ...
  templateUrl: './editoracao.component.html',
  styleUrl: './editoracao.component.css'
})
export class EditoracoesComponent {
  ...
  countSections(curriculum: ICurriculum): number {
    return curriculum.curriculum.editoracoes.length
  }

  getWorks(): void {
    this.sections = this.curriculumns.reduce<Editoracao[]>(
      (trabalhos, curriculum) =>
        trabalhos.concat(
          curriculum.curriculum.editoracoes.map((section) => ({
            ...section,
            ano: section.ano,
