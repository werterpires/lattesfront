...
import { ApresentacaoDeTrabalho } from 'src/app/shared/services/objTypes'
...
@Component({
  selector: 'app-apresentacao-de-trabalho',
  standalone: true,
  imports: [
    NgFor,
    NgIf,
  ...
  templateUrl: './apresentacao-de-trabalho.component.html',
  styleUrl: './apresentacao-de-trabalho.component.css'
})
export class ApresentacoesDeTrabalhoComponent {
  curriculumns: ICurriculum[] = []
  sections: ApresentacaoDeTrabalho[] = []
  tableElements: ITableElements[] = [
    {
      title: 'Apresentação de Trabalho',
      property: 'apresentacaoDeTrabalho'
    }
  ]
  ...
  countSections(curriculum: ICurriculum): number {
    return curriculum.curriculum.apresentacoesDeTrabalho.length
  }

  getWorks(): void {
    this.sections = this.curriculumns.reduce<ApresentacaoDeTrabalho[]>(
      (trabalhos, curriculum) =>
        trabalhos.concat(
          curriculum.curriculum.apresentacoesDeTrabalho.map((section) => ({
            ...section,
            ano: section.ano,
