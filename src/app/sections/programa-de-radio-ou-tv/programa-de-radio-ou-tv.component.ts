...
import { ProgramaDeRadioOuTV } from 'src/app/shared/services/objTypes'
...
@Component({
  selector: 'app-programa-de-radio-ou-tv',
  standalone: true,
  imports: [
    NgFor,
    NgIf,
  ...
  templateUrl: './programa-de-radio-ou-tv.component.html',
  styleUrl: './programa-de-radio-ou-tv.component.css'
})
export class ProgramasDeRadioOuTVComponent {
  curriculumns: ICurriculum[] = []
  sections: ProgramaDeRadioOuTV[] = []
  tableElements: ITableElements[] = [
    {
      title: 'Programas de rádio ou TV',
      property: 'programasDeRadioOuTV'
    }
  ]
  ...
  countSections(curriculum: ICurriculum): number {
    return curriculum.curriculum.programasDeRadioOuTV.length
  }

  getWorks(): void {
    this.sections = this.curriculumns.reduce<ProgramaDeRadioOuTV[]>(
      (trabalhos, curriculum) =>
        trabalhos.concat(
          curriculum.curriculum.programasDeRadioOuTV.map((section) => ({
            ...section,
            ano: section.ano,
