...
import { ParticipationProps, ParticipationsWorkKey } from '../types'
...
export class QualitativeEventsWorksComponent {
  ...
  orderNow() {
    const propKey = this.orderProp as ParticipationsWorkKey
  ...
  changeAllShowFilterToFalse(key: string) {
    const oneProp = key as ParticipationsWorkKey
    for (const prop of this.eventProps) {
      if (prop.key !== oneProp) {
