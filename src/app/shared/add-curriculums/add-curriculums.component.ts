...
export class AddCurriculumsComponent {
  ...
  parseXml(xmlString: string): any {
    try {
      ...
      this.alertService.showAlerts('error', 'Erro ao adicionar currículo', [
        'Ocorreu um erro ao tentar cadastrar o currículo.'
      ])
    }
  }
  ...
  async makeCreateCurriculumDto(curr: any): Promise<ICreateCurriculum | null> {
    ...
    const artigos = {
      aceitos: producaoBibliografica['ARTIGOS-ACEITOS-PARA-PUBLICACAO'],
      publicados: producaoBibliografica['ARTIGOS-PUBLICADOS']
    }
  ...
  onFileSelected(event: any): void {
    ...
    if (files && files.length > 0) {
      for (let i = 0; i < files.length; i++) {
        ...
        if (files[i].name.split('.')[1] === 'xml') {
          this.readFile(files[i], files[i].name)
        }
      }
    }
